...
class PyOlcEngine3D:
    ...
    def render(self, tri, matWorld, matView, ScreenWidth, ScreenHeight):
        # print("Render Call by" + str(os.getpid()))
    ...
    def on_update(self, ElapsedTime, pool):
        ...
        # vLeft = (self.vLookDir + 90) * (8 * ElapsedTime)
        ...
        results = [pool.apply_async(self.render, args=(tri, matWorld, matView, custom_fast_turtle.ScreenWidth(), custom_fast_turtle.ScreenHeight())) for tri in self.meshCube.tris]
        vecTrianglesToRaster.append(list([p.get() for p in results]))
        # print(vecTrianglesToRaster)

        vecTrianglesToRaster = list(itertools.chain.from_iterable(itertools.chain.from_iterable(vecTrianglesToRaster)))
        # print(vecTrianglesToRaster)
...
if __name__ == '__main__':
    with Pool(processes=100) as pool:
        ...
        while True:
            ...
            win.on_update(timed, pool)
            custom_fast_turtle.update()
            """ timed = time.time() - start_time
            # print(f"{win.vCamera.y=}")
